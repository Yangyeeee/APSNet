...
# Download dataset for point cloud reconstruction. For each Mesh model of Shape-Net-Core download 1 point-cloud
# with 2048 points sampled uniformly at random (around 1.4GB)
BASE_DIR = osp.dirname(osp.dirname(os.path.abspath(__file__)))
DATA_DIR = os.path.join(BASE_DIR, "data")
if not os.path.exists(DATA_DIR):
    os.mkdir(DATA_DIR)

zipfile = "shape_net_core_uniform_samples_2048" + ".zip"
if not os.path.exists(os.path.join(DATA_DIR, zipfile[:-4])):
    www = "https://www.dropbox.com/s/vmsdrae6x5xws1v/" + zipfile + "?dl=0"
    os.system("wget %s" % www)
    os.system("mv %s %s; unzip %s" % (zipfile + "\?dl\=0", zipfile, zipfile))
...
def load_all_point_clouds_under_folder(
    top_dir, n_threads=20, file_ending=".ply", verbose=False
):
    ...
    return PointCloudDataSet(
        pclouds, labels=syn_ids + "_" + model_ids, init_shuffle=False
    )


def load_and_split_all_point_clouds_under_folder(
    top_dir,
    n_threads=20,
    file_ending=".ply",
    ...
    model_ids_train, model_ids_val, model_ids_test, _ = split_data(
        model_ids, split, seed, perm
    )
    syn_ids_train, syn_ids_val, syn_ids_test, _ = split_data(syn_ids, split, seed, perm)

    ds_train = PointCloudDataSet(
        pclouds_train, labels=syn_ids_train + "_" + model_ids_train, init_shuffle=False
    )
    ds_val = PointCloudDataSet(
        pclouds_val, labels=syn_ids_val + "_" + model_ids_val, init_shuffle=False
    )
    ds_test = PointCloudDataSet(
        pclouds_test, labels=syn_ids_test + "_" + model_ids_test, init_shuffle=False
    )
...
def load_point_clouds_from_filenames(file_names, n_threads, loader, verbose=False):
    ...
    if verbose:
        print(
            "{0} pclouds were loaded. They belong in {1} shape-classes.".format(
                len(pclouds), len(np.unique(class_ids))
            )
        )
...
class PointCloudDataSet(data.Dataset):
    ...
    def __init__(
        ...
        Output:
            original_pclouds, labels, (None or Feed)
        """
        ...
        if labels is not None:
            assert (
                point_clouds.shape[0] == labels.shape[0]
    ...
    def __getitem__(self, idx):
        ...
        current_points = self.point_clouds[idx].copy()
        ...
        if self.noisy_point_clouds is None:
            ...
            noise_points = self.transforms(self.noisy_point_clouds[idx])
            return (current_points, label, noise_points)
    ...
    def merge(self, other_data_set):
        ...
        labels_1 = self.labels.reshape([self.num_examples, 1])
