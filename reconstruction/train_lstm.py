...
def test(model, sampler, loader,epoch):
    mean_loss = []
    ...
    if epoch%10 == 0:
    #     i = 2500
        # points = torch.tensor(loader.dataset.point_clouds[i]).unsqueeze(0)
        # points = points.cuda()
        # sampler = sampler.eval()
        # p0_simplified, p0_projected = sampler(points)
        # sampled_points = p0_projected.transpose(2, 1)
        ...
        plot_3d_point_cloud(
            p0_projected[0].detach().cpu(),points[0].detach().cpu(), c="#cfcfcf",in_u_sphere=True,  elev=77,azim=-90,title="LSTM sampled points {}".format(args.num_out_points),epoch=epoch
        )
        plot_3d_point_cloud(
...
def eval(args):
    ...
    try:
        ...
        snet_checkpoint =torch.load( "./log/{}/checkpoints/best_lstm.pth".format(args.log_dir))
    ...
    with torch.no_grad():
        ...
        if args.vis:
            ...
            plot_3d_point_cloud(
                org = rec[0].detach().cpu(),
                in_u_sphere=True, elev=77,azim=-90,
                title="LSTM Reconstruction {}, NRL {:.2f}".format(args.num_out_points,loss_n.item()),
            )
            ...
            for j, data in tqdm(enumerate(testDataLoader), total=len(testDataLoader)):
                ...
                p0_simplified, p0_projected = sampler(points)
                sampled_points = p0_projected.transpose(2, 1)
                # sampled_points = sampler(points).transpose(2, 1)
...
def main(args):
    ...
    def compute_samplenet_loss(sampler, data, epoch):
        ...
        samplenet_loss = args.alpha * simplification_loss # + args.lmbda * sampler.tmp
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            loss = test(PointAE.eval(),sampler, valDataLoader,epoch)
            np.save("./att/att_{}".format(epoch), sampler.p.cpu().numpy())
            print('Test loss', loss.item())
            ...
            if (loss <= best):
                ...
                savepath = str(checkpoints_dir) + '/best_lstm.pth'
...
if __name__ == '__main__':
    ...
    if args.test == 0:
        ...
        for num in nums:
            ...
            args.log_dir = "lstm_i" + str(num)
            args.sess = "lstm_i_out{}".format(args.num_out_points)
            print(args)
            res = main(args)
            loss.append(res)
            print(loss)
