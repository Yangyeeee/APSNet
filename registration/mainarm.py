...
def options(argv=None, parser=None):
    ...
    parser.add_argument("--alpha", type=float, default=1, help="Simplification regularization loss weight [default: 0.01]")
    ...
    parser.add_argument('--k1', type=float, default=1, help='arm slop')
    parser.add_argument('--ar', action='store_true', default=False, help='use ar')
...
class Action:
    ...
    def train_1(self, model, trainloader, optimizer, device, epoch):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            model.sampler.loss = 0

            writer.add_scalar('loss/K', model.sampler.k1, epoch*len(trainloader)*epoch + i)
            if model.sampler.training:
                model.sampler.forward_mode = True
                ...
                if model.sampler is not None and model.sampler.name == "samplenet":
                    ...
                    a.append(model.sampler.num.item())
                    loga.append(model.sampler.m)
                ...
                pcrnet_loss2, pcrnet_loss_info = self.compute_pcrnet_loss(
                    model, sampled_data, device, epoch
                )

                l0_grad = model.sampler.grad * self.l0
                loss = model.sampler.loss * self.l0
                model.sampler.eval()
                if self.ar is not True:
                    model.sampler.forward_mode = False
                    if model.sampler is not None and model.sampler.name == "samplenet":
                        (
                            sampler_loss1,
                            sampled_data1,
                    ...
                    pcrnet_loss1, pcrnet_loss_info1 = self.compute_pcrnet_loss(
                        model, sampled_data1, device, epoch
                    )
                else:
                    pcrnet_loss1 = 0
                    sampler_loss1 = 0
                model.sampler.f1 = pcrnet_loss1 + sampler_loss1
                model.sampler.f2 = pcrnet_loss2 + sampler_loss

                model.sampler.train()
                if epoch >= 100:
                    k1 = model.sampler.k1
                    k1 +=  (model.sampler.loss - 0.0625)
            ...
            # # SampleNet loss is already factorized by ALPHA and LMBDA hyper parameters.pcrnet_loss2  +

            optimizer.zero_grad()
            if model.sampler.training:
                grad = model.sampler.update_phi_gradient()
                model.sampler.loga.backward(gradient=(grad+l0_grad) , retain_graph=False)
            optimizer.step()

            vloss1 = (loss + pcrnet_loss2 + sampler_loss).item()
            vloss += vloss1
            loss1 += loss.item()
            loss2 += pcrnet_loss2.item()
            loss3 += sampler_loss.item()
        ...
        ave_l0 = float(loss1) / count
        ave_pcrnet = float(loss2) / count
        ave_sampler_loss = float(loss3) / count

        writer.add_scalar('loss/pcrnet_loss', ave_pcrnet, epoch)
        writer.add_scalar('loss/sampler_loss', ave_sampler_loss, epoch)
        writer.add_scalar('loss/L0', ave_l0, epoch)
