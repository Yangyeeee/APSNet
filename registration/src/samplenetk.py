...
class SampleNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        y = torch.max(a, 2)[0]  # Batch x 128
        deltas = a - y.unsqueeze(-1)
        # dist = torch.sum(deltas ** 2, dim=1)* -1

        dist = self.bn6(self.conv6(deltas)).squeeze()
        A = self.sampler(dist)
        A = torch.softmax(dist+A,dim=-1)
        ...
        # Matched points
            ...
            match = batched_index_select(x, 2, ind).permute(0, 2, 1)
