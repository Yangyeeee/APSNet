...
class SampleNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor,epoch):
        ...
        if self.training:
            ...
            m = l0_test(loga, 0, 1)
        y = x * m.unsqueeze(1)
        self.m = (m > 0).squeeze().sum(-1).float().mean()
        self.loss = get_loss2(loga).sum()
        self.loga = m.view(-1)
        # m = self.sampler(loga)
        # y = x * m.unsqueeze(1)
        ind = torch.topk(loga, 64, dim=-1)[1]
        self.ind  = ind
        #
        # Simplified points
        simp = y #batched_index_select(y, 2, ind)
        match = None
        proj = None
